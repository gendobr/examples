...
logDir = "file:///D:/data/proj/sandbox/spark/data/logs/"
sc = SparkContext("local[*]", "LogReader")
ssc = StreamingContext(sc, 10)

# defining the checkpoint directory
ssc.checkpoint("D:/data/proj/sandbox/spark/tmp")
...
logData = lines.map(parse_log_line).reduceByKey(line_join).map(message_join).saveAsTextFiles("D:/data/proj/sandbox/spark/data/streming-out/logs")
